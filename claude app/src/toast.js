(function(){
  ...
  function classify(message){
    ...
    if (/(error|failed|could not|invalid|corrupt|missing|no |cannot|can't)/.test(m)) return 'error';
  ...
  window.appNotify = function(message, options){
    ...
    el.className = 'app-toast ' + (type === 'error' ? 'is-error' : type === 'warning' ? 'is-warning' : '');
    var title = options.title || (type === 'error' ? 'Notice' : type === 'warning' ? 'Heads up' : 'Panel Designer');
